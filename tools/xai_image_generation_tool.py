...
def _check_xai_image_available() -> bool:
    ...
    # Check credential pool first (for managed instances).
    try:
...
def _resolve_xai_credentials():
    """Return (api_key, base_url) for xAI, checking credential pool first."""
    try:
        from agent.auxiliary_client import (
            _select_pool_entry, _pool_runtime_api_key, _pool_runtime_base_url,
        )
        pool_present, entry = _select_pool_entry("xai")
        if pool_present:
            key = _pool_runtime_api_key(entry)
            base = _pool_runtime_base_url(entry, "https://api.x.ai/v1") or "https://api.x.ai/v1"
            if key:
                return key, base
    except Exception:
        pass
...
def xai_image_generate(
    ...
    try:
        ...
        return json.dumps({"success": True, "image": str(local_path)})
    except Exception as dl_err:
        logger.warning("Could not download xAI image, returning URL: %s", dl_err)
        return json.dumps({"success": True, "image": remote_url})
