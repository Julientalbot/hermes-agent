...
PLATFORM_HINTS = {
    ...
    "telegram": (
        ...
        "You can send media files AND documents natively: to deliver any file "
        "to the user, include MEDIA:/absolute/path/to/file in your response. Images "
        "(.png, .jpg, .webp) appear as photos, audio (.ogg) sends as voice "
        "bubbles, videos (.mp4) play inline, and every other file type "
        "(.pdf, .pptx, .docx, .xlsx, .html, .zip, etc.) arrives as a "
        "downloadable document. Whenever you create or edit a file for the user, "
        "deliver it this way — include its MEDIA:/absolute/path; never just state "
        "the server file path, the user cannot access the gateway's filesystem. "
    ...
    "discord": (
        ...
        "attachments, audio as file attachments, and every other file type "
        "(.pdf, .pptx, .docx, .xlsx, .html, .zip, etc.) as a downloadable "
        "document. Whenever you create or edit a file for the user, deliver it "
        "this way — never just state the server file path, the user cannot "
        "access the gateway's filesystem. You can also include image URLs "
