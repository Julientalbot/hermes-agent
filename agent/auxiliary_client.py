...
class _CodexCompletionsAdapter:
    ...
    def _build_responses_kwargs(self, kwargs: Dict[str, Any]) -> Tuple[Dict[str, Any], str, Any]:
        ...
        if isinstance(extra_body, dict):
            # Grok 4.6 accepts Priority Processing; preserve the main transport's carve-out.
            service_tier = extra_body.get("service_tier")
            from agent.model_metadata import is_grok_46_family
            xai_priority = is_grok_46_family(model) and service_tier == "priority"
            if isinstance(service_tier, str) and service_tier.strip() and (not is_xai or xai_priority):
