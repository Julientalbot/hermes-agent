...
class TestMediaDeliveryFailureSurfacing:
    """Undeliverable MEDIA files are reported, not dropped silently."""
    ...
    def test_notice_dedupes_name_in_both_lists(self):
        # A file rejected before send AND reported as a send failure is named once.
        notice = BasePlatformAdapter.build_media_failure_notice(
            ["report.pptx"], [("report.pptx", "boom")]
        )
        assert notice is not None
        assert notice.count("report.pptx") == 1
        assert "couldn't attach" in notice
